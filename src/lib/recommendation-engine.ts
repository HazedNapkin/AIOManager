...
export async function buildCreatorsRails(
    seeds: SeedItem[],
    signal?: AbortSignal,
    railSize = 20,
    watchedTmdbIds?: Set<string>
): Promise<RankedRail[]> {
    const resolveSeeds = seeds.slice(0, 20)
    const topSeeds = resolveSeeds.slice(0, 5)
    if (topSeeds.length < 2) return []

    const { proxyFetch } = await import('../api/metadata/adapters/tmdb.ts')

    const watchedSet = new Set<string>(watchedTmdbIds)
    for (const seed of resolveSeeds) {
        const tm = seed.itemId.match(/^tmdb:(\d+)$/i)
        if (tm) watchedSet.add(`tmdb:${tm[1]}`)
    }

    interface CreatorInfo { id: number; name: string; count: number; job: string; mediaTypes: Set<'movie' | 'tv'> }

    const creatorMap = new Map<number, CreatorInfo>()

    for (let si = 0; si < resolveSeeds.length; si++) {
        if (signal?.aborted) break
        const seed = resolveSeeds[si]
        try {
            const imdbId = seed.itemId.startsWith('tt') ? seed.itemId : null
            const tmdbMatch = seed.itemId.match(/^tmdb:(\d+)$/i)
            if (!imdbId && !tmdbMatch) continue

            let tmdbId: number | null = tmdbMatch ? Number(tmdbMatch[1]) : null
            const mediaType = seed.type === 'series' || seed.type === 'anime' ? 'tv' : 'movie'

            if (!tmdbId && imdbId) {
                const findData = await proxyFetch<any>(`find/${encodeURIComponent(imdbId)}?external_source=imdb_id`, signal)
                if (!findData) continue
                const result = mediaType === 'tv' ? findData?.tv_results?.[0] : findData?.movie_results?.[0]
                tmdbId = result?.id
            }
            if (!tmdbId) continue

            watchedSet.add(`tmdb:${tmdbId}`)

            if (si >= 5) continue

            const details = await proxyFetch<any>(`${mediaType}/${tmdbId}?append_to_response=credits`, signal)
            if (!details) continue

            const crew = Array.isArray(details.credits?.crew) ? details.credits.crew : []
            for (const member of crew) {
                if (member.job === 'Director' && member.id) {
                    const existing = creatorMap.get(member.id)
                    if (existing) { existing.count++; existing.mediaTypes.add(mediaType); continue }
                    creatorMap.set(member.id, { id: member.id, name: member.name || 'Unknown', count: 1, job: 'Director', mediaTypes: new Set([mediaType]) })
                }
            }

            const cast = Array.isArray(details.credits?.cast) ? details.credits.cast.slice(0, 3) : []
            for (const member of cast) {
                if (member.id) {
                    const existing = creatorMap.get(member.id)
                    if (existing) { existing.count++; existing.mediaTypes.add(mediaType); continue }
                    creatorMap.set(member.id, { id: member.id, name: member.name || 'Unknown', count: 1, job: 'Cast', mediaTypes: new Set([mediaType]) })
                }
            }
        } catch {}
    }

    const recurring = Array.from(creatorMap.values())
        .filter(c => c.count >= 2)
    ...
    for (const creator of recurring) {
        if (signal?.aborted) break
        try {
            const items: ScoredRecommendation[] = []
            const seenIds = new Set<string>()

            const creditsData = await proxyFetch<any>(
                `person/${creator.id}/combined_credits`,
                signal
            )
            if (creditsData) {
                const credits: any[] = []
                if (Array.isArray(creditsData.cast)) credits.push(...creditsData.cast)
                if (creator.job === 'Director' && Array.isArray(creditsData.crew)) {
                    credits.push(...creditsData.crew.filter((c: any) => c.job === 'Director'))
                }

                for (const r of credits) {
                    if (signal?.aborted) break
                    if (!r.poster_path) continue
                    if ((r.vote_count ?? 0) < 10) continue
                    const dedupeKey = `tmdb:${r.id}`
                    if (watchedSet.has(dedupeKey)) continue
                    if (seenIds.has(dedupeKey)) continue
                    seenIds.add(dedupeKey)
                    items.push({
                        id: { tmdb: r.id, slug: `tmdb:${r.id}` },
                        ...
                        year: r.release_date ? parseInt(r.release_date.slice(0, 4), 10) : r.first_air_date ? parseInt(r.first_air_date.slice(0, 4), 10) : undefined,
            ...
            const railItems = items.slice(0, railSize)
            if (railItems.length > 0) {
                rails.push({
                    title: `More from ${creator.name}`,
                    source: 'recommendations',
                    items: railItems,
                })
            }
...
export async function buildThemedRails(
    ...
    const themes: Array<{ label: string; genres: string[] }> = []
    themes.push({ label: sortedGenres[0], genres: [sortedGenres[0]] })
    for (let i = 1; i < sortedGenres.length && themes.length < 3; i++) {
        themes.push({
            label: `${sortedGenres[0]} & ${sortedGenres[i]}`,
            genres: [sortedGenres[0], sortedGenres[i]],
        })
    }
