                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
                    <div>
                        <motion.h2
                            ...
                            className="text-5xl md:text-7xl font-bold text-white tracking-[-0.04em] leading-tight"
                        >
                            Top Titles
                        </motion.h2>
                        <p className="text-white/70 font-bold mt-2 uppercase text-sm">The content that defined your journey</p>
                    </div>
                        ...
                        <div className="flex p-1.5 border border-white/10 rounded-full bg-white/5 shadow-xl">
                            {['year', 'all-time'].map((v) => (
                                <button
                                    ...
                                    className={`px-6 py-2 rounded-full text-xs font-semibold uppercase transition-all ${view === v ? 'bg-white text-black' : 'text-white/40 hover:text-white'
                                        }`}
                                >
                                    {v === 'year' ? (
                ...
                <div className="flex flex-col gap-6 max-w-5xl mx-auto w-full">
                    {hero && (
                        <motion.div
                            ...
                            whileInView={{ opacity: 1, y: 0 }}
                            viewport={{ once: true, margin: "-100px" }}
                            className="bg-gradient-to-br from-[#2a1a08] to-[#151310] pb-0 p-8 md:p-12 rounded-xl flex flex-col items-center relative border border-white/10 shadow-2xl cursor-pointer group"
                            onClick={() => handleTitleClick(hero)}
                        >
                            <div className="absolute top-8 left-8 bg-yellow-500 text-black px-4 py-1 rounded-full text-xs font-semibold uppercase shadow-lg z-20">
                                #1 Most Watched
                            </div>

                            <div className="text-center space-y-4 relative z-10 mb-12 mt-8">
                                <h3 className={`text-5xl md:text-7xl font-bold text-white tracking-[-0.04em] leading-tight ${isLight ? 'drop-shadow-lg' : 'drop-shadow'}`}>
                                    {hero.name}
                                </h3>
                                <div className="flex items-center justify-center mt-2">
                                    <span className="text-xs font-bold uppercase px-3 py-1 rounded-full bg-white/10 text-white/70 border border-white/10">
                                ...
                                <p className="text-white/70 text-xs md:text-sm font-bold uppercase mt-4 max-w-md mx-auto">
                                    {Math.round((hero.totalHours / currentData.totalHours) * 100)}% of your total stream time
                                </p>
                            </div>

                            <div className="relative w-52 md:w-72 aspect-[2/3] rounded-xl overflow-hidden shadow-2xl shadow-black/80 mt-auto transform group-hover:-translate-y-4 transition-transform duration-500 border border-white/10">
                                <Poster
                                    src={hero.poster}
                                    itemId={hero.itemId}
                                    itemType={hero.type}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-20 mt-4">
                        {rest.map((title, i) => (
                            ...
                            >
                                ...
                                >
                                    <div className="w-8 shrink-0 font-bold text-2xl italic transition-colors text-center relative z-10" style={{ color: i === 0 ? '#fbbf24' : i === 1 ? '#94a3b8' : i === 2 ? '#cd7f32' : 'rgba(255,255,255,0.15)' }}>
                                        {i + 2}
                                    </div>
                                    ...
                                    <div className="flex-1 min-w-0 pr-4 relative z-10">
                                        ...
                                        <div className="flex flex-col items-start gap-1 mt-1">
                                            <span className="text-xs font-bold text-white/70 uppercase">{Math.round(title.totalHours)} Hrs</span>
                                            <span className="text-xs font-bold uppercase px-2 py-0.5 rounded-full bg-white/10 text-white/70 border border-white/10">
